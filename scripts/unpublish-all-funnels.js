(async () => {
  ...
  console.log('Unpublishing ' + funnelIds.length + ' funnels...');

  for (var i = 0; i < funnelIds.length; i++) {
    try {
      var res = await fetch('/api/funnel/' + funnelIds[i] + '/publish', {
        method: 'POST',
  ...
  console.log('Done! Unpublished ' + success + '/' + funnelIds.length + ' funnels (' + errors + ' errors)');
})();
